...
export const SUBMIT_OTHER_WALL_POST_SUCCESS = 'SUBMIT_OTHER_WALL_POST_SUCCESS';
export const SUBMIT_OTHER_WALL_POST_FAILURE = 'SUBMIT_OTHER_WALL_POST_FAILURE';
...
export function getOtherWallPosts(dispatch, token, uri_end) {
  ...
  return function (dispatch)  {
    ...
    .then((response) => {
      dispatch(getOtherWallPostsSuccess(response.data)) && console.log('GETTING FIRST SET OF POSTS AGAIN AFTER A OTHER WALL POST SUBMISSION')
    })
    .catch(error => {
       dispatch(getOtherWallPostsFailure(error.response.data)) && console.log(error.response.data.error)
    })
    };
}


export function submitOtherWallPostFetch(dispatch, token, postText) {
...
export function submitOtherWallPost(dispatch, token, postText, id) {
  ...
  return function (dispatch)  {
    ...
    .then((response) => {
      dispatch(getOtherWallPosts(dispatch, token, `/api/user/${id}/wall_posts`))
    })
    .catch(error => {
       dispatch(submitOtherWallPostFailure(error)) && console.log('ERROR ! ! !', error)
    })
    };
}
...
export function submitOtherWallPostSuccess(data) {
  return {
...
export function submitOtherWallPostFailure(data) {
  return {
